...
class InvalidStateException(WorkflowException):
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(message, status_code=409, error_code="INVALID_STATE", details=details)
...
async def re_execute_node(node_id: int, request: ExecutionRequest, service: NodeService = Depends(get_node_service)):
...
async def retry_node(node_id: int, request: ExecutionRequest, service: NodeService = Depends(get_node_service)):
...
def create_workflow(workflow_data: WorkflowCreate, service: WorkflowService = Depends(get_workflow_service)):
...
def list_workflows(
    ...
    return service.get_workflows_paginated(skip, limit)
...
def update_workflow(
    ...
    return service.update_workflow(workflow_id, update_data)
...
async def start_workflow(workflow_id: int, service: WorkflowService = Depends(get_workflow_service)):
...
def get_workflow_staleness(workflow_id: int, service: WorkflowService = Depends(get_workflow_service)):
...
class ExecutionSimulator:
    ...
    async def _execute_varl(self, node: NodeInstance, inputs: Dict[str, Any], feedback: Optional[str]) -> Dict[str, Any]:
        ...
        if node.definition_id == "3.1.2":
            ...
            artifact["raw_results"] = "Simulated Raw Data"
            artifact["vv_data"] = "Simulated V&V Data"
            artifact["sensitivity_data"] = "Simulated Sensitivity Data"
    ...
    async def _execute_sca(self, node: NodeInstance, inputs: Dict[str, Any], feedback: Optional[str]) -> Dict[str, Any]:
        ...
        if node.definition_id == "1.1.2":
            ...
            if feedback:
                candidates.append({KEY_ID: "C3", "name": "Option based on feedback", "description": "F_data"})

        analysis = f"Comparative analysis of {len(candidates)} candidates."
        output[KEY_CANDIDATES] = candidates
...
class HITLService:
    ...
    async def _approve_and_proceed(self, node: NodeInstance, interaction_data: Optional[Dict[str, Any]]):
        ...
        if execute_next:
            await self.node_service.enqueue_initial_execution(next_node.id)
            return {
...
class NodeService:
    ...
    async def enqueue_initial_execution(self, node_id: int) -> NodeInstance:
        """Start the first execution for a not-started node."""
        node = self.get_node_instance(node_id)
    ...
    async def enqueue_retry(self, node_id: int, user_feedback: Optional[str] = None) -> NodeInstance:
        """Retry a failed node."""
        node = self.get_node_instance(node_id)
...
class WorkflowService:
    ...
    def get_workflows_paginated(self, skip: int, limit: int) -> PaginatedResponse[WorkflowInstanceRead]:
        """Return workflows ordered by creation date with pagination."""
        total = self.db.query(WorkflowInstance).count()
        workflows = (
    ...
    def create_workflow(self, create_data: WorkflowCreate) -> WorkflowInstance:
    ...
    def update_workflow(self, workflow_id: int, update_data: WorkflowUpdate) -> WorkflowInstance:
    ...
    async def start_workflow(self, workflow_id: int) -> NodeInstance:
        ...
        return await node_service.enqueue_initial_execution(first_node.id)

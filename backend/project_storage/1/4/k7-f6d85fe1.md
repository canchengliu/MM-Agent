...
async def redis_pubsub_listener():
    ...
    logger.info("Subscribed to Redis channel '%s' for workflow events.", settings.WEBSOCKET_BROADCAST_CHANNEL)
    try:
        while True:
            ...
            try:
                ...
                logger.error("Discarding malformed Redis payload: %s", raw_data)
                continue

            workflow_id = payload.get("workflow_id")
            if workflow_id is None:
                logger.warning("Dropping event without workflow_id: %s", payload)
                continue
...
async def execute_node_task(ctx: Dict[str, Any], node_id: int, **kwargs: Any) -> None:
    ...
    logger.info("Worker received task to execute node %s with args: %s", node_id, kwargs)
    db = SessionLocal()
    try:
        node = db.query(NodeInstance).get(node_id)
        if not node:
            logger.error("Node %d not found in database. Discarding job.", node_id)
            return

        if node.status != NodeStatus.EXECUTING:
            logger.warning(
                "Worker picked up job for node %d, but its status is '%s'. Discarding stale job.",
                node_id,
                node.status,
            )
            return
    ...
    except Exception as exc:  # noqa: BLE001 - we need to capture and log all worker failures
        logger.error("Worker task for node %d failed permanently: %s", node_id, exc, exc_info=True)
        fail_db = None
        try:
            ...
            logger.critical(
                "CRITICAL: Failed to update node %d status to FAILED after task error: %s",
                node_id,
                update_exc,
                exc_info=True,
            )
        ...
        db.close()
    logger.info("Worker finished task for node %s", node_id)
...
async def on_job_failure(ctx: Dict[str, Any], job: Any, exc: BaseException) -> None:
    ...
    logger.error("Job %s for node %s failed permanently: %s", job.function, job.args[0], exc)
...
# Fix 3.3: Add granular external input inheritance control (inherits_external_inputs).
# Fix 3.4: Ensure outputs include all artifacts required downstream.
PHASE_2_TEMPLATE = {
    ".2.1.1": {
    ...
    ".2.2.2": {
        ...
        "outputs": [
            ...
            "Selected Visualizations for Task i",
        ],
        "inherits_external_inputs": False,
    },
}
...
class NodeVersion(Base):
    ...
    hitl_history = Column(MutableDict.as_mutable(JSON), nullable=False)
...
class TemporaryExecutionResult(Base):
    ...
    input_dependencies = Column(MutableDict.as_mutable(JSON), nullable=False)
    accumulated_hitl_interactions = Column(MutableDict.as_mutable(JSON), nullable=False)
    llm_model_name = Column(String, nullable=False)
    temperature = Column(Float, nullable=False)
...
class HITLService:
    ...
    async def _approve_and_proceed(self, node: NodeInstance, interaction_data: Optional[Dict[str, Any]]):
        ...
        try:
            ...
            self.db.rollback()
            logger.error("Transaction failed during approve_and_proceed: %s", exc, exc_info=True)
            raise InvalidStateException(
                f"Failed to finalize approval and advance workflow due to error: {exc}"
            )
    ...
    def _determine_final_output(
        ...
        if node.hitl_mode == HITLMode.SCA:
            ...
            return {
                ...
                "summary": f"User selected {len(selected_items)} options.",
            }
...
class NodeService:
    ...
    async def enqueue_execution(
        ...
        logger.info("Enqueued execution for node %s", node_id)
        return node
    ...
    async def enqueue_hitl_action(
        ...
        self.db.refresh(node)
        await self._broadcast_node_update(node)
        logger.info("Enqueued HITL continuation for node %s", node_id)
        return node
    ...
    async def execute_in_worker(
        ...
        try:
            ...
            logger.error("Failed during execution preparation for node %s: %s", node_id, exc, exc_info=True)
            self.db.rollback()
            raise
    ...
    def _validate_execution_request(
        ...
        if node.status == NodeStatus.EXECUTING:
            logger.warning(
                "Re-executing node %s which was in EXECUTING state. Assuming previous execution crashed.",
                node.id,
            )
...
class WorkflowService:
    ...
    def handle_generator_node_completion(self, node: NodeInstance, output: Dict[str, Any]) -> bool:
        ...
        if not taskbook:
            logger.warning("Generator node %s completed without a valid Structured Modeling Taskbook.", node.id)
            return False
